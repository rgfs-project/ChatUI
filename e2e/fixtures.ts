 ...
 * cannot touch. Podman only: Docker has no such flag and needs none.
 *
...
function startContainer(env: ServerEnv, dataDir: string): ServerHandle {
  ...
  /*
   * `--userns=keep-id` is Podman's, and Docker rejects the flag outright — so
   * asking for `E2E_CONTAINER_ENGINE=docker` used to fail before the image was
   * even started. Under Docker the daemon runs as root and writes the bind
   * mount as whatever uid the container user has, which the fixtures can read;
   * it is only rootless Podman that needs the mapping.
   *
   * `:z` relabels the bind mount for SELinux, which Podman needs on Fedora and
   * RHEL and which Docker also understands.
   */
  const rootless = CONTAINER_ENGINE.includes('podman');
  const mount = [
    ...(rootless ? ['--userns=keep-id'] : []),
    '--network=host',
    '-v',
    `${dataDir}:/data${rootless ? ':z' : ''}`,
  ];
