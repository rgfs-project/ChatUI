...
/**
 * The environment as it arrives when nobody set `LLAMA_BASE_URL`.
 *
 * `configured: false` is the whole of "nobody set it": the base URL still has
 * its default value, and acting on a default is what invented providers that
 * were never there.
 */
const BOOTSTRAP = { baseUrl: 'http://127.0.0.1:8080', timeoutMs: 120_000, configured: false };
...
describe('providers.json', () => {
   ...
   */
  it('configures nothing when the file is absent and nobody named a provider', async () => {
   ...
   *
   * A deployment whose only configuration channel is a compose file has no way
   * to name a provider before signing in, and `LLAMA_BASE_URL` is documented
   * as the way it is done. Acting on a value the operator actually set is not
   * the same as acting on a default, and only the second one invents things.
   */
  it('writes the provider the operator named, on a first run', async () => {
    const result = await registry().load({
      baseUrl: 'http://llama.internal:8080',
      timeoutMs: 120_000,
      configured: true,
    });

    expect(result.providers.map((p) => p.baseUrl)).toEqual(['http://llama.internal:8080']);

    // Through the same file the admin panel edits, so it can be seen and
    // changed afterwards rather than being invisible state.
    const written = JSON.parse(
      await readFile(join(paths.systemDir(), 'providers.json'), 'utf8')
    ) as { providers: { id: string; baseUrl: string }[] };
    expect(written.providers[0]?.baseUrl).toBe('http://llama.internal:8080');
  });

  it('never overwrites a file that already exists', async () => {
    await writeConfig([entry({ id: 'chosen', name: 'Chosen' })]);
    ...
    // The operator's own file wins, so a provider edited in the admin panel is
    // not undone by the next restart.
    expect(result.providers.map((p) => p.id)).toEqual(['chosen']);
  });
