...
export class ProviderRegistry {
  ...
  /**
   * Loads the file, writing it once from the environment if it is absent and
   * the operator named a provider.
   *
   * `configured` is the whole of the distinction. Bootstrapping from the
   * *default* base URL is what this used to do, and it gave every fresh
   * instance a provider at `127.0.0.1:8080` that was usually not there —
   * permanently unavailable, in the first place an operator looks. Doing it
   * from a URL somebody actually set is the opposite: `LLAMA_BASE_URL` is
   * documented as how a container is pointed at a model server, and a
   * deployment whose only configuration channel is the compose file has no
   * other way to say it before signing in.
   *
   * Once. The file is authoritative afterwards — this never overwrites it, so
   * a provider edited in the admin panel is not undone by a restart, and
   * removing the last provider does not resurrect one.
   *
  ...
  async load(bootstrap: {
    ...
    if (raw === null && bootstrap.configured) {
      raw = await this.#writeBootstrap(bootstrap);
    }

    if (raw === null) {
      this.#logger.info(
        'No providers configured; set LLAMA_BASE_URL, add one via the admin panel, or edit _system/providers.json',
        {}
      );
  ...
  /**
   * Writes the provider the environment named, and returns what was written.
   *
   * Through the same file the admin panel edits, rather than a parallel
   * in-memory path, so that what a first run produces is something an operator
   * can then see, change and remove in the usual way. A failure here is
   * logged and survived: an unwritable `_system` is a problem, but not one
   * worth refusing to start over.
   */
  async #writeBootstrap(bootstrap: {
    baseUrl: string;
    apiKey?: string;
    timeoutMs: number;
  }): Promise<string | null> {
    ...
    try {
      await ensureDir(this.#paths.systemDir());
      await atomicWriteFile(this.#file(), `${file}\n`);
      // The URL, never the key.
      this.#logger.info('Wrote the first provider from LLAMA_BASE_URL', {
        baseUrl: bootstrap.baseUrl,
      });
      return file;
    } catch (error) {
      this.#logger.error('Could not write the first provider from LLAMA_BASE_URL', { error });
      return null;
    }
  }
}
