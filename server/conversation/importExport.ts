 ...
 *
 * Generous against a real export — the largest are tens of megabytes of JSON
 * across a few hundred files — and finite, which is the property that was
 * missing.
 */
...
/**
 * How much of the archive is handed to the decompressor at a time.
 *
 * This is what makes the limit above an actual ceiling rather than a report.
 * Output arrives in response to input, so the largest amount that can appear
 * between two chances to check is this slice multiplied by deflate's maximum
 * ratio — 1032:1, a property of the format and not something the sender can
 * exceed. At 8 KiB that is about 8 MiB of overshoot in the worst case, on top
 * of a budget measured in hundreds.
 *
 * Feeding the whole archive at once, by contrast, produces the whole member in
 * a single chunk: the check then runs on bytes that have already been
 * allocated, which is a report of what happened rather than a limit on it.
 */
const PUSH_SLICE_BYTES = 8 * 1024;

/**
 * Opens an archive, reading only what an import can use.
 *
 * Two things bound the work, and neither of them believes the archive.
 *
 * Members that are not `.json` are never started, so they are never inflated —
 * an import has no use for them, and the cheapest bytes are the ones that are
 * never produced. What is started is counted as it arrives, in pieces, and the
 * moment the running total passes the budget the read stops.
 *
 * The sizes declared in a zip's headers are not consulted for any of this. They
 * are the sender's word about their own file: an earlier version of this
 * checked them, which reads like a limit and is not one, because an archive can
 * say whatever it likes about what it contains. Counting what actually comes
 * out needs no such assumption — an archive whose headers claim ten bytes and
 * expand to forty megabytes is refused at exactly the same point as an honest
 * one.
 */
function unpack(bytes: Uint8Array, limits: ImportLimits): Record<string, Uint8Array> {
  ...
  /*
   * The refusal, kept where it can be thrown again.
   *
   * Refusing means throwing from inside a decompressor callback, and the
   * decompressor treats that as the stream having gone wrong: it reports the
   * failure back through the same callback, where it would otherwise be
   * rethrown as "this archive could not be read". Remembering the real reason
   * is what keeps the reader being told their archive is too big rather than
   * that it is corrupt.
   */
  let refusal: AppError | null = null;
  const refuse = (error: AppError): never => {
    // Thrown from a local of its own: `refusal` is reassigned from inside the
    // callbacks below, which is enough to stop it being seen as an error type
    // at the point it would be thrown.
    const chosen = refusal ?? error;
    refusal = chosen;
    throw chosen;
  };

  const unzip = new Unzip((file) => {
    // Never started, so fflate never decompresses it.
    if (!file.name.toLowerCase().endsWith('.json')) return;

    entries += 1;
    if (entries > maxEntries) {
      refuse(
        AppError.validation(`That archive contains too many files to import (limit ${maxEntries}).`)
      );
    }
    ...
    file.ondata = (error, chunk, final) => {
      ...
      if (error !== null) {
        throw AppError.validation('That archive could not be read.');
      }

      inflated += chunk.length;
      if (inflated > maxTotalBytes) {
        refuse(AppError.validation('That archive is too large to import once expanded.'));
      }
      parts.push(chunk);

      if (final) files[file.name] = concatChunks(parts);
    };
    file.start();
  });
  ...
  try {
    for (let at = 0; at < bytes.length; at += PUSH_SLICE_BYTES) {
      const end = Math.min(at + PUSH_SLICE_BYTES, bytes.length);
