...
export function useScrollPin(): ScrollPin {
  ...
  const onContentChange = useCallback((): void => {
    ...
    /*
     * Released, but sitting at the bottom anyway — re-pin.
     *
     * `release` fires the moment a reply outgrows its reserve, which is also
     * the moment the view is still exactly at the bottom; it says "stop
     * following", not "the reader left". No `scroll` event follows a reply
     * that then stays short or a reasoning block that collapses back down, so
     * without this the flag never learns the transcript settled where it
     * already was — and offers a jump-to-latest arrow over nothing to jump
     * to. `onScroll` re-pins on every real scroll that lands at the bottom;
     * this is that same rule for a position that changed under the reader
     * instead of one they scrolled to.
     */
    if (isAtBottom(element)) {
      pinnedRef.current = true;
  ...
  }, [measureScrollable, scrollToBottom, isAtBottom]);
