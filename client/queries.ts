...
/**
 * Which account the cache currently belongs to.
 *
 * A counter rather than a user id: what matters is not who is signed in but
 * *that it changed*, and a counter cannot accidentally compare equal across a
 * sign-out and back in as the same person — which is still a new session, with
 * a new server-side state to read.
 *
 * It exists because clearing the cache on a change is not enough on its own. A
 * mutation started by one account finishes whenever the server gets round to
 * it, and its callbacks write to the cache when they run, not when they were
 * scheduled: a rename that resolves after somebody else has signed in puts the
 * previous account's conversation into the new one's cache, and a failed
 * mutation's rollback puts back a snapshot taken before the change. Cancelling
 * queries does not touch either — a mutation is not a query.
 *
 * So every mutation records the account it began under, and every callback that
 * writes checks it is still the one in force. The result of work belonging to a
 * previous session is discarded rather than applied.
 */
let currentAccountEpoch = 0;

/** Called when the signed-in account changes; see the note above. */
export function beginAccountEpoch(): void {
  currentAccountEpoch += 1;
}

export function accountEpoch(): number {
  return currentAccountEpoch;
}

/** Stamps a mutation with the account it started under. */
export interface AccountOwned {
  epoch: number;
}

function owned(): AccountOwned {
  return { epoch: currentAccountEpoch };
}

/**
 * Whether a callback is running for an account that has since been replaced.
 *
 * Missing context counts as stale: a mutation whose `onMutate` never ran has
 * nothing to vouch for it, and guessing in its favour is the failure this
 * guards against.
 */
function fromPreviousAccount(context: AccountOwned | undefined): boolean {
  return context === undefined || context.epoch !== currentAccountEpoch;
}
...
> {
  ...
  return useMutation({
    ...
    onMutate: () => owned(),
    onSuccess: (_result, { conversationId }, context) => {
      if (fromPreviousAccount(context)) return;
...
export function usePinConversation(): UseMutationResult<
  ...
  AccountOwned & { previous: ConversationSummary[] | undefined }
> {
  const client = useQueryClient();
  return useMutation({
...
> {
  ...
  return useMutation({
    ...
    onMutate: () => owned(),
    onSuccess: (detail, _variables, context) => {
      if (fromPreviousAccount(context)) return;
...
export function useDeleteArtifact(): UseMutationResult<void, Error, string, AccountOwned> {
  const client = useQueryClient();
  return useMutation({
...
export function useDeleteConversation(): UseMutationResult<void, Error, string, AccountOwned> {
  const client = useQueryClient();
  return useMutation({
...
> {
  ...
  return useMutation({
    ...
    onMutate: () => owned(),
    onSuccess: (detail, { conversationId }, context) => {
      if (fromPreviousAccount(context)) return;
...
export interface SendContext extends AccountOwned {
...
> {
  ...
  return useMutation({
    ...
    onMutate: async ({ conversationId, content, attachmentIds }) => {
      ...
      if (previous !== undefined) {
        // The ids are already known client-side — they finished uploading
        // before Send was even enabled — so there is no reason for the
        // attachment to wait for the server's copy of this message to show
        // up. `MessageAttachments` resolves each id to its own metadata, so
        // the bare id list here is all rendering needs.
      ...
      return { ...owned(), conversationId, temporaryId, previous };
    },

    onSuccess: (accepted, _variables, context) => {
      if (fromPreviousAccount(context) || context === undefined) return;
