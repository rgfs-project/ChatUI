...
describe('session expiry mid-use', () => {
  it('moves to signed out, keeps the draft, and returns to the conversation', async () => {
    ...
    await screen.findByRole('button', { name: /Sign in/ });
    // The notice now arrives as a toast, fired from an effect rather than
    // rendered synchronously alongside the form — an extra tick behind it.
    await waitFor(() => expect(screen.getByRole('status').textContent).toContain('session ended'));
