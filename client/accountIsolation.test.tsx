...
import { accountEpoch, createQueryClient } from './queries.ts';
import {
  conversationsBody,
  installTestServer,
  modelsBody,
...
describe('signing out', () => {
  ...
  /**
   * The shell has to announce the change, not merely clean up after it: work
   * already in flight is stamped with the account it began under, and that
   * stamp is what lets its callbacks discard themselves later
   * (see staleMutation.test.tsx for what happens when they do not).
   */
  it('announces the change, so work already in flight can disown itself', async () => {
    mount();
    await arriveAs('alice', 'Alice private notes');
    await screen.findByText('Alice private notes');
    ...
    expect(accountEpoch()).toBeGreaterThan(before);
  });
