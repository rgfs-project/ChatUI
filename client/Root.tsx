...
function Shell(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    /*
     * Announced before anything is cleared, so that work already in flight is
     * stamped as belonging to the account that is going away. Clearing alone
     * left a gap a mutation could walk through: cancelling stops queries, but a
     * mutation is not a query, and its callbacks write to the cache when they
     * run rather than when they were scheduled — a rename resolving after the
     * next person signs in put the previous account's conversation into their
     * cache, and a failed one's rollback put back a snapshot from before the
     * change. Those callbacks now check this and discard their result.
     */
    beginAccountEpoch();
