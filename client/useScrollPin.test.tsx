...
describe('useScrollPin', () => {
  ...
  describe('jump to latest', () => {
    ...
    /**
     * `release` fires the instant a reply outgrows its reserve — which is
     * also the instant the view is still exactly at the bottom, not the
     * instant the reader left. If the reply then stays that length (or a
     * reasoning block that pushed it there collapses back down), nothing
     * ever scrolls: no `scroll` event tells the flag the transcript settled
     * where it already was, and the arrow was left pointing at nothing below
     * the composer to jump to.
     */
    it('is not offered once release finds the transcript still at the bottom', () => {
      const harness = mount();
      // A token already streamed in, which is what gives `scrollable` its
      // first real measurement — mounting alone measures against geometry
      // from before the stub is applied.
      act(() => harness.pin.onContentChange());
      expect(harness.element.scrollTop).toBe(1500); // still at the bottom

      act(() => harness.pin.release());
      expect(harness.pin.showJumpToLatest).toBe(true); // more may still be coming

      // The generation that triggered `release` has settled without moving
      // the view — the geometry is exactly what it was.
      act(() => harness.pin.onContentChange());
    ...
    it('keeps the arrow after release when content really is left below the fold', () => {
      const harness = mount();
      act(() => harness.pin.onContentChange());
      ...
      expect(harness.pin.pinned).toBe(false);
      expect(harness.pin.showJumpToLatest).toBe(true);
      // Still not moved out from under the reader.
      expect(harness.element.scrollTop).toBe(1500);
    });
  });
});
